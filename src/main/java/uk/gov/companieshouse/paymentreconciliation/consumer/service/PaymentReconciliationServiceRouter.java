...
public class PaymentReconciliationServiceRouter {
    ...
    public void route(payment_processed paymentReconciliation) {
        ...
        if (paymentSessionOptional.isEmpty()) {
            LOGGER.info("Payment not found or skipped for PaymentResourceId: %s"
                    .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
            return;
        }
        ...
        if (paymentDetails == null) {
            LOGGER.info("Payment details not found for PaymentResourceId: %s"
                    .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
        ...
        if (isReconcilable(paymentSession)) {
            LOGGER.info("Reconciling payment processed message for payment id: %s"
                    .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
            PaymentUtils.maskSensitiveFields(paymentSession, productCodeLoader);
            if (isRefundTransaction(paymentReconciliation)) {
                LOGGER.info("Handling refund transaction for payment id: %s"
                        .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
                ...
                LOGGER.info("Handling standard transaction for payment id: %s"
                        .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
                standardTransactionHandler.handle(paymentDetails, paymentSession);
            } else {
                LOGGER.info("Payment processed message has unhandled status for payment id: %s"
                        .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
            }
        } else {
            LOGGER.info("Payment processed message is not reconcilable for payment id: %s"
                    .formatted(paymentReconciliation.getPaymentResourceId()), DataMapHolder.getLogMap());
        }
    }
    ...
    private boolean isReconcilable(PaymentResponse paymentSession) {
        ...
        if (paymentSession.getCosts().getFirst().getClassOfPayment() == null
                || paymentSession.getCosts().getFirst().getClassOfPayment().isEmpty()) {
            return false;
        }
